#!/bin/bash
# Ralph Loop - Generated 2026-01-10T22:00:00Z
# Project: ralph
...
handle_stuck_exit() {
  ...
  echo "  3. Manually fix the issue and run: ./ralph/loop.sh (will preserve attempt counts)"
  echo "  4. Skip this story by manually updating sprint-status.yaml"
  echo "  5. Reset and start fresh with: ./ralph/loop.sh --restart"
  echo ""

  log_warning "State preserved in ralph/config.json for debugging"
}
  ...
  if [[ -n "$PROGRESS_FILE" ]]; then
    {
      ...
      echo "State: Saved to config.json - resume with ./ralph/loop.sh"
    } >> "$PROGRESS_FILE" 2>/dev/null || true
  fi

  echo ""
  log_warning "Loop interrupted. State saved - run ./ralph/loop.sh to resume from iteration ${iteration:-unknown}."
  echo ""
...
echo ""
echo "To continue where you left off: ./ralph/loop.sh"
echo "To start fresh: ./ralph/loop.sh --restart"
exit 2
