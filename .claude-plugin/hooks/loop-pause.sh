#!/usr/bin/env bash
#
# BMAD Ralph Plugin - Loop Pause Hook
# Executed when loop execution is paused
#
# Usage: ./loop-pause.sh [LOOP_NAME]
#
# Environment:
...
#   RALPH_CUSTOM_LOOP_PAUSE - Optional custom script to execute after built-in actions
...
log() {
  local level="$1"
  shift
  echo "[$(date -u +"%Y-%m-%dT%H:%M:%SZ")] [loop-pause] [$level] $*" | tee -a "$LOG_FILE" >&2
}
...
if [[ -d "ralph/loops/$LOOP_NAME" ]]; then
  ...
  log "ERROR" "Cannot pause archived loop '$LOOP_NAME'"
  exit 1
else
...
log "INFO" "Loop pause hook triggered for $LOOP_NAME"
...
if [[ "$START_TIME" != "null" ]]; then
  ...
  PAUSE_SECONDS=$(date -j -f "%Y-%m-%dT%H:%M:%SZ" "$PAUSE_TIME" "+%s" 2>/dev/null || date +"%s")
  DURATION=$((PAUSE_SECONDS - START_SECONDS))
  DURATION_HOURS=$((DURATION / 3600))
  DURATION_MINS=$(((DURATION % 3600) / 60))
  DURATION_STR="${DURATION_HOURS}h ${DURATION_MINS}m"
else
...
log "INFO" "Duration: $DURATION_STR"

# Update config.json with pause time
TMP_FILE=$(mktemp)
jq --arg time "$PAUSE_TIME" '.stats.pausedAt = $time' "$CONFIG_FILE" > "$TMP_FILE"
mv "$TMP_FILE" "$CONFIG_FILE"
log "INFO" "Updated config.json with pause time: $PAUSE_TIME"

# Log lifecycle event to progress.txt
if [[ -f "$PROGRESS_FILE" ]]; then
  {
    echo ""
    echo "## Loop Pause Event"
    echo "Timestamp: $PAUSE_TIME"
    echo "Loop: $LOOP_NAME"
  ...
  log "INFO" "Logged loop pause event to progress.txt"
fi
...
if [[ -n "${RALPH_CUSTOM_LOOP_PAUSE:-}" ]]; then
  if [[ -f "$RALPH_CUSTOM_LOOP_PAUSE" ]] && [[ -x "$RALPH_CUSTOM_LOOP_PAUSE" ]]; then
    log "INFO" "Executing custom loop pause script: $RALPH_CUSTOM_LOOP_PAUSE"
    ...
    export RALPH_DURATION="$DURATION_STR"

    if "$RALPH_CUSTOM_LOOP_PAUSE"; then
      log "INFO" "Custom script executed successfully"
    else
      log "WARN" "Custom script failed (exit code: $?)"
    fi
  else
    log "WARN" "Custom script not found or not executable: $RALPH_CUSTOM_LOOP_PAUSE"
  fi
fi

# Output summary
echo -e "${YELLOW}⏸ Loop pause hook completed successfully${NC}"
...
echo -e "  Duration: ${BLUE}$DURATION_STR${NC}"
